...
const candidatoNaoEncontrado = computed(
  () =>
    numeroVoto.value.length === quantidadeDigitos &&
...
function concluirVoto(tipo: TipoVoto) {
  ...
  void audioConfirmacao.play().catch(() => undefined);
...
<template>
  <main class="estrutura-pagina">
    ...
    <AvisoPrivacidade
      texto="Seu voto é secreto e armazenado somente neste terminal."
    />
  </main>
</template>
