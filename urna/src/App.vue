...
const currentScreen = ref<'setup' | 'voting' | 'success'>('setup')
...
const lastVoteType = ref<'candidate' | 'blank'>('candidate')
...
const runningMate = computed(() => {
  ...
  // Prefer explicit fields (name_vice / party_vice) when provided by apuração
  if (cand.name_vice) {
    return {
      name: cand.name_vice.trim() || 'Vice não informado',
      party: (cand.party_vice || '').trim(),
    }
  }
...
function handlePhysicalKeyboard(event: KeyboardEvent) {
  if (currentScreen.value !== 'voting') return
...
<template>
  <main v-if="currentScreen === 'setup'" class="page-shell provision-page">
  ...
  <main v-else-if="currentScreen === 'voting'" class="page-shell">
